/* Online-mode annotation (see annotate.h). Runs after parsing; does not parse.
 *
 * Given an already-parsed module and its source path, builds the prompt, runs
 * Bob once (Bob reads the file itself), and stores each returned flowchart into
 * the symbol whose starting line matches. Bob is invoked with the prompt as a
 * single argument (never a shell), so its backticks/quotes/newlines are safe.
 ...
 * (each with its start line). malloc'd; caller frees. Portable. */
 ...
 * into the symbol whose starting line is N (the match). Returns how many
 * symbols were filled. */
...
int zdoc_ai_annotate(const char *path, Module *mod, const char *bob_cli)
{
    if (!path || !mod)
        return -1;
    const char *cli = (bob_cli && *bob_cli) ? bob_cli : "bob";
