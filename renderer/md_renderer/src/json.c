/*
 * Generic JSON reading primitives - no knowledge of md_renderer's own
 * schema lives here (that's md_model.c). Kept separate so this file could
...
void *xmalloc(size_t n) {
    void *p = malloc(n ? n : 1);
    if(!p) {
        fprintf(stderr, "zdoc-md-renderer: out of memory\n");
        exit(1);
    }
    return p;
}
...
/* Every numeric field in md_model.c's schema (line, parent_index,
